...
trait ReCaptcha{
   ...
   * @codeAsDoc
   */
  //protected def reCaptchaOptions = ("theme" -> "red") ~ ("lang" -> S.containerRequest.flatMap(_.locale).map(_.getLanguage).getOrElse("en"))
  protected def reCaptchaOptions = ("theme" -> "custom") ~ ("lang" -> S.containerRequest.flatMap(_.locale).map(_.getLanguage).getOrElse("en")) ~ ("custom_theme_widget" -> "recaptcha_widget")
    ...
    */
    ...
    <xml:group>
      ...
      <div id="recaptcha_widget" style="display:none">
        ...
        <!--beta-->
        <!--<span class="recaptcha_only_if_image"><lift:loc>recaptcha.instructions</lift:loc></span>-->
        ...
        <div class="form-group">
          ...
          <div data-lift="Msg?id=reCaptcha;errorClass=errorRecaptcha"></div>
        </div>        

        <div class="btn-group btn-group-justified">
          <div class="btn-group">
            <a href="javascript:Recaptcha.reload()" class="btn btn-white btn-lg" type="button">
              <span class="glyphicon glyphicon-refresh"></span>
            </a> 
          </div>
          <div class="btn-group">
            <a href="javascript:Recaptcha.switch_type('audio')" class="btn btn-white btn-lg" type="button">
              <span class="glyphicon glyphicon-volume-up"></span>
            </a> 
          </div>
          <div class="btn-group">
            <a href="javascript:Recaptcha.showhelp()" class="btn btn-white btn-lg" type="button">
  ...
  protected def validateCaptcha(): List[FieldError] = {
    ...
    res match {
      case Failure(msg, _, _) =>
        List(FieldError(FakeFieldIdentifier(Full("reCaptcha")), msg))
      case Full(msg) =>
        List(FieldError(FakeFieldIdentifier(Full("reCaptcha")), msg))
      case Empty => Nil
    }
