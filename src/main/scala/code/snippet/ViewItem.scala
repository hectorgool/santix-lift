...
object ViewItem{
  ...
  val viewItemParam = Menu.param[Items]("viewItemParam", "viewItemParam", 
    Items.findSlug _, _.slug.get 
    ) / "item" / *

  val picsItemParam = Menu.param[Items]("picsItemParam", "picsItemParam", 
    Items.findSlug _, _.slug.get 
    ) / "item" / * / "pics" 

  //lazy val profileLoc = viewItemParam.toLoc
